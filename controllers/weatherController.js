...
export const weatherForecast = async (req, res) => {
  ...
  try {
    ...
    // Ensure these fields exist on trip model
    const { destinationCity, destinationCountry, startDate } = trip;

    const weatherData = await fetchWeather(destinationCity, destinationCountry, startDate);
